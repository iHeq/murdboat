export function getLocation(rawText) {
  ...
  if (rawText[0] === '{') {
    try {
      ...
      if (jsonData['server']) {
        botLocation['server'] = jsonData['server'];
        validJSON = true;
      } else {
        throw 'Error: Not /locraw';
      }

      // Overrides
      if (jsonData['lobbyname']) {
        botLocation['lobbyname'] = jsonData['lobbyname'];
      } else {
        botLocation['lobbyname'] = null;
      }
      if (jsonData['gametype']) {
        botLocation['gametype'] = jsonData['gametype'];
      } else {
        botLocation['gametype'] = null;
      }
      if (jsonData['map']) {
        botLocation['map'] = jsonData['map'];
      } else {
        botLocation['map'] = null;
      }
